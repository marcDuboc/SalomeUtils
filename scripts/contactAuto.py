...
# 5494 8FA2 ADF2 CAF0 4A5C A315 CA FF65
...
class Contact():
    def __init__(self,id,part,surface):
        self.id = id
        self.type = "bonded"  # bonded, sliding, gap
    ...
    def cyl_prop_to_dict(self, shape):
        ...
        if str(p[0]) in ('CYLINDER2D', 'CYLINDER'):
            c_prop = dict(type=str(p[0]), center=(p[1], p[2], p[3]), direction=(
                p[4], p[5], p[6]), diameter=p[7], length=0)
            return c_prop
        return False
